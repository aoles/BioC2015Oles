...
<font style="font-size:90%">Implemented using the constant time algorithm [@Perreault2007].</font>
...
- immunologic filter composed of B cells, T cells, dendritic cells (DC) and macrophages
...
<font class=scaledmath>$$N(p, a)$$</font>

- Local intensity
<font class=scaledmath>$$\lambda(p) = \lim_{a\rightarrow 0} \frac{E[ N(p, a)]}{a}$$</font>

- For a *stationary* process, i.e., homogeneous all over the region
<font class=scaledmath>$$\lambda(p) = \text{const.}$$</font>
Then the intensity in an area $A$ is proportional to the area
<font class=scaledmath>$$E(N(\cdot, A)) = \lambda A$$</font>
...
<font class=scaledmath>$$G(r) = P(W \leq r) = 1-e^{-\lambda \pi r^2}$$</font>
...
<div class=scaledmath>$$K_{\text{inhom}}(r) = \sum_{i,j} {\mathbf 1}_{d(p_i, p_j) \le r} \times \frac{e(p_i, p_j, r)} { \lambda(x_i)  \lambda(x_j) },$$
