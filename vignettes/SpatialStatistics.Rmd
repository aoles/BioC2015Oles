...
- For a *stationary* process, i.e., homogenous all over the region
$$\lambda(p) = \text{const.}$$
...
<font style="font-size: 90%;">where $K$ is the Gaussian smoothing kernel, and $e(p_i)$ is an edge correction factor.</font>
...
- no dependency between occurences of objects in non-overlapping regions
- $N(p, A)$ follows a Poisson distribution with rate $\lambda A$
</div>

### The nearest neighbour (NN) distance distribution function $G$

Cumulative distribution function of the distance $W$ from a typical random point to its nearest neigbour. For a Poisson process
